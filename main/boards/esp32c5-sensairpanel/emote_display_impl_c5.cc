...
static std::atomic<bool> g_gfx_output_enabled(false);
...
static void InitializeGraphics(lv_display_t* disp, gfx_handle_t* engine_handle)
{
    gfx_core_config_t gfx_cfg = {
        ...
        .buffers = {
            .buf1 = nullptr,
            .buf2 = nullptr,
            .buf_pixels = DISPLAY_WIDTH * DISPLAY_HEIGHT,
        },
    ...
    void* ps2 = heap_caps_malloc(fb_size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (ps1 && ps2) {
        gfx_cfg.buffers.buf1 = ps1;
        gfx_cfg.buffers.buf2 = ps2;
        gfx_cfg.buffers.buf_pixels = DISPLAY_WIDTH * DISPLAY_HEIGHT;
        gfx_cfg.flags.buff_dma = false;
    } else {
        if (ps1) heap_caps_free(ps1);
        if (ps2) heap_caps_free(ps2);
        gfx_cfg.buffers.buf1 = nullptr;
        gfx_cfg.buffers.buf2 = nullptr;
        gfx_cfg.buffers.buf_pixels = DISPLAY_WIDTH * 16;
        gfx_cfg.flags.buff_dma = true;
...
static void InitializeEyeAnimation(gfx_handle_t engine_handle, mmap_assets_handle_t assets_handle)
{
    ...
    gfx_obj_align(obj_anim_eye, GFX_ALIGN_LEFT_MID, 40, -10);
    gfx_anim_set_mirror(obj_anim_eye, true, 50);
...
static void InitializeLabels(gfx_handle_t engine_handle)
{
    ...
    gfx_obj_align(obj_label_tips, GFX_ALIGN_TOP_MID, 25, 45);
    gfx_obj_set_size(obj_label_tips, 160, 40);
    gfx_label_set_text(obj_label_tips, "Starting...");
...
EmoteEngine::EmoteEngine(lv_display_t* disp)
{
    ESP_LOGI(TAG, "Create EmoteEngine with LVGL display: %p", disp);

    InitializeAssets(&assets_handle_);
    InitializeGraphics(disp, &engine_handle_);
    disp_ = disp;
...
{
    if (!g_gfx_output_enabled.load()) {
        ESP_LOGD(TAG, "GFX draw skipped: LVGL mode active");
        gfx_emote_flush_ready(handle, true);
        return;
    }
    auto* disp = static_cast<lv_display_t*>(gfx_emote_get_user_data(handle));
    if (!disp) {
        ESP_LOGE(TAG, "GFX flush failed: LVGL display is null");
        gfx_emote_flush_ready(handle, true);
        return;
    }
    auto panel = static_cast<esp_lcd_panel_handle_t>(lv_display_get_user_data(disp));
    if (!panel) {
        ESP_LOGE(TAG, "GFX flush failed: panel handle is null");
        gfx_emote_flush_ready(handle, true);
        return;
    }
    if (lvgl_port_take_trans_sem(disp, 0) != ESP_OK) {
        ESP_LOGW(TAG, "GFX draw skipped: trans sem busy");
        gfx_emote_flush_ready(handle, true);
        return;
    }
    ESP_ERROR_CHECK_WITHOUT_ABORT(esp_lcd_panel_draw_bitmap(panel, x_start, y_start, x_end, y_end, color_data));
    if (lvgl_port_take_trans_sem(disp, portMAX_DELAY) != ESP_OK) {
        ESP_LOGE(TAG, "GFX draw timeout waiting trans sem");
    }
    ESP_ERROR_CHECK_WITHOUT_ABORT(lvgl_port_give_trans_sem(disp, false));
    gfx_emote_flush_ready(handle, true);
}

EmoteDisplay::EmoteDisplay(lv_display_t* disp)
{
    disp_ = disp;
    InitializeEngine(disp);
}
...
void EmoteDisplay::SetStatus(const char* status)
{
    ...
    if (std::strcmp(status, "聆听中...") == 0) {
        ESP_LOGI(TAG, "Wakeword detected, entering GFX mode");
        EnterGfxMode();
...
void EmoteDisplay::InitializeEngine(lv_display_t* disp)
{
    engine_ = std::make_unique<EmoteEngine>(disp);
}
...
void EmoteDisplay::EnterGfxMode()
{
    if (!disp_) return;
    ESP_LOGI(TAG, "Switching to GFX mode");
    g_gfx_output_enabled.store(true);
}

void EmoteDisplay::EnterLvglMode()
{
    if (!disp_) return;
    ESP_LOGI(TAG, "Switching to LVGL mode");
    g_gfx_output_enabled.store(false);
}
