...
typedef struct {
    ...
    TaskHandle_t     done_notify_task; /* Notify this task when action is handled */
    ...
    while (true) {
        if (xQueueReceive(adc_data->worker_queue, &msg, portMAX_DELAY)) {
            ...
            case ADC_MIC_ACTION_DESTROY: {
                ...
                if (msg.done_notify_task) {
                    xTaskNotifyGive(msg.done_notify_task);
                }
...
static int _adc_data_enable(const audio_codec_data_if_t *h, esp_codec_dev_type_t dev_type, bool enable)
{
    ...
    adc_mic_msg_t msg = {
        ...
        .done_notify_task = xTaskGetCurrentTaskHandle(),
